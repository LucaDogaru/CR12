...
$name_err = $price_err = $description_err = $longitude = $latitude = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_latitude )){
        $price_err = "Please enter latitude .";     
    } else{
    ...
    if(empty($name_err) && empty($price_err) && empty($description_err) && empty($longitude_err) && empty($latitude_err)){
        ...
        $sql = "INSERT INTO mount (name, price, description, longitude, latitude) VALUES (?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
           
            mysqli_stmt_bind_param($stmt, "sssss", $param_name, $param_price, $param_description, $param_longitude, $param_latitude,);
